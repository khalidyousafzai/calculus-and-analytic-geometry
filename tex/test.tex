...
\ابتدا{سوال}
رداس \عددی{\SI{10}{\centi\meter}} کے دائرے پر کتنی لمبائی کی قوس  (الف) \عددی{\tfrac{4\pi}{5}} ریڈیئن (ب) \عددی{110^{\circ}} کا وسطی زاویہ بنائے گی؟
\انتہا{سوال}
\ابتدا{جواب}
\wf{\unexpanded{
...
%===========================
\ابتدا{سوال}
رداس \عددی{8} کے دائرے پر \عددی{10\pi} لمبائی کی قوس، مرکز پر کتنا وسطی زاویہ بناتی ہے؟ جواب درجات اور ریڈیئن میں تلاش کریں۔ 
\انتہا{سوال}
%=========================
...
%===========================
\ابتدا{سوال}\موٹا{کیلکولیٹر}\quad
ایک میٹر قطر کے پہیے کو ہموار زمین پر\عددی{\SI{30}{\centi\meter}} چلایا جاتا ہے۔پہیا کتنا زاویہ گھوما ہو گا؟ جواب (الف)  ریڈیئن کے دسواں حصہ اور (ب)  درجے  کے ایک حصہ درستگی تک تلاش کریں۔
\انتہا{سوال}
%=============================
\موٹا{تکونیاتی تفاعل کی قدر پیمائی}

\ابتدا{سوال}
درج ذیل بایاں جدول مکمل کریں۔ کیلکولیٹر یا جدول سے جوابات پڑھنے کی اجازت نہیں۔
\begin{align*}
\begin{array}{cccccc}
...
%=====================
\ابتدا{سوال}
درج بالا دایاں جدول مکمل کریں۔ کیلکولیٹر یا جدول سے جوابات پڑھنے کی اجازت نہیں۔
\انتہا{سوال}
%========================
...
\ابتدا{جواب}
\wf{\unexpanded{
دوری عرصہ \عددی{\pi} ہے۔\\
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[axis lines=middle,width=5cm,xtick={90,180},
xticklabels={\llap{$\tfrac{\pi}{2}$},\rlap{$2\pi$}},ytick={-1,1},xlabel={$x$},ylabel={$y$},ymin=-1.2,ymax=1.2,xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south}]
\addplot[domain=-90:240,samples=100]{sin(2*x)};
\draw(axis cs:150,1)node[font=\small,xshift=-0.5em]{$y=\sin 2x$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,axis lines=middle,width=5cm,xtick={1,2},ytick={-1,1},xlabel={$x$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},ymin=-1.2,ymax=1.2,xmax=3.5]
\addplot[domain=-1.2:2.5,samples=100]{cos(180*x)};
\draw(axis cs:1,1)node[font=\small,xshift=1em,yshift=0.5em]{$y=\cos \pi x$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,,axis lines=middle,width=5cm,xtick={3,6},ytick={-1,1},xlabel={$x$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},ymin=-1.2,ymax=1.2]
\addplot[domain=-1:7,samples=100]{-sin(180/3*x)};
\draw(axis cs:2,1)node[font=\small,shift={(2em,0.5em)}]{$y=-\sin \tfrac{\pi x}{3}$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,,axis lines=middle,width=5cm,xtick={180,360},
xticklabels={\llap{$\pi$},\rlap{$2\pi$}},ytick={-1,1},xlabel={$x$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},ymin=-1.2,ymax=1.2]
\addplot[domain=-45:460,samples=100]{cos(x-180/2)};
\draw(axis cs:275,1)node[font=\small,above,xshift=1em]{$y=\cos(x-\tfrac{\pi}{2})$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,,axis lines=middle,width=5cm,xtick={45,135,315},xticklabels={$\tfrac{\pi}{2}$,$\tfrac{3\pi}{4}$,$\tfrac{7\pi}{4}$},ytick={1,2},xlabel={$x$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},enlargelimits]
\addplot[domain=-90:400,samples=100]{sin(x-45)+1};
\draw(axis cs:200,2.2)node[font=\small,xshift=1em]{$y=\sin(x-\tfrac{\pi}{4})+1$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
سوال \حوالہ{سوال_ابتدا_ترسیم_تشاکل_الف} تا سوال \حوالہ{سوال_ابتدا_ترسیم_تشاکل_ب} میں دیے تفاعل کو \عددی{ts} مستوی میں ترسیم کریں جہاں افقی محور \عددی{t} ہے۔ہر تفاعل کا دوری عرصہ اور تشاکل تلاش کریں۔
...
\ابتدا{جواب}
\wf{\unexpanded{ 
دائرہ کار:\عددی{\tfrac{\pi}{2}}، \\
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,,axis lines=middle,width=5cm,xtick={\empty},ytick={-2,-1,1,2},xlabel={$t$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},ymin=-2.2,ymax=2.2,xmin=-200,xmax=220,enlarge y limits=0.2]
\draw[dashed](axis cs:-180,-2.2)--(axis cs:-180,2.2)node[pos=0.5,below,fill=white,font=\small]{$-\pi$};
\draw[dashed](axis cs:-90,-2.2)--(axis cs:-90,2.2)node[pos=0.5,below,fill=white,font=\small]{$-\tfrac{\pi}{2}$};
\draw[dashed](axis cs:90,-2.2)--(axis cs:90,2.2)node[pos=0.5,below,fill=white,font=\small]{$\tfrac{\pi}{2}$};
\draw[dashed](axis cs:180,-2.2)--(axis cs:180,2.2)node[pos=0.5,below,fill=white,font=\small]{$\pi$};
...
\draw(axis cs:180,1)node[fill=white,font=\small]{$\cos 2t$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[axis lines=middle,height=4cm,width=5cm,xtick={-3,-2,-1,1,2,3},ytick={1},xlabel={$t$},ylabel={$y$},xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},ymin=-3.2,ymax=3.2,xmin=-3.2,xmax=4]
...
\draw(axis cs:3,1)node[fill=white,font=\small,xshift=-0.5em]{$y=\sec \tfrac{\pi t}{2}$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
%====================
\ابتدا{سوال}
اگر سوال \حوالہ{سوال_ابتدا_اضافی_مماثل_پ} میں \عددی{B=A} پُر کیا جائے تب کیا حاصل ہو گا؟ کیا آپ حاصل کردہ مماثل کو پہلے سے جانتے ہیں؟ 
\انتہا{سوال}
%=======================
\ابتدا{سوال}
...
\عددی{\tan(A+B)=\tfrac{\tan A+\tan B}{1-\tan A\tan B}} ہے۔یہ  کلیہ   اخذ کریں۔
\انتہا{سوال}
%================
\ابتدا{سوال}
...
%=========================
\ابتدا{سوال}
قاعدہ کوسائن کو شکل \حوالہ{شکل_سوال_ابتدا_دائرہ_کوسائن} کے طرز کی شکل پر لاگو کرتے ہوئے \عددی{\cos (A+B)} کا کلیہ اخذ کریں۔یہ شکل کیسا ہو گا۔ 
\انتہا{سوال}
%===========================
...
%===========================
\ابتدا{سوال}\ترچھا{کیلکولیٹر}\quad
ایک مثلث کا ضلع \عددی{c=2} اور زاویے  \عددی{A=\tfrac{\pi}{4}} اور \عددی{B=\tfrac{\pi}{3}} ہیں۔زاویہ \عددی{A} کا مخالف ضلع \عددی{a}  تلاش کریں۔
\انتہا{سوال}
\ابتدا{جواب}
\wf{\unexpanded{
$a=1.464$
...
\quad
\عددی{x} کی چھوٹی قیمتوں کے لئے \عددی{\sin x \approx x} ہوتا ہے جہاں \عددی{x} کی ناپ ریڈیئن میں ہے۔اس کی وجہ حصہ \حوالہ{حصہ_استعمال_خط_بندی_اور_تفرقات} میں بتلائی جائے گی۔  ( اپنی تسلی کے لئے کیلکولیٹر کی مدد سے \عددی{0.001}   اور اس کے سائن کی قیمتوں کا موازنہ کریں۔) \عددی{\abs{x}<0.1} کے لئے تخمینی خلل \عددی{5000} میں \عددی{1} حصہ سے کم ہو گا۔\\
(الف)\quad
...
\ابتدا{جواب}
\wf{\unexpanded{
\عددی{A=2,B=2\pi,C=-\pi,D=-1}؛\\
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,tiny,axis lines=middle,width=5cm,ytick={-3,1},xlabel={$x$},ylabel={$y$},ymin=-3.2,ymax=1.5,xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},xtick={-90,90,270,540},xticklabels={$-\tfrac{\pi}{2}$,$\tfrac{\pi}{2}$,$\tfrac{3\pi}{2}$,$\tfrac{5\pi}{2}$}]
\addplot[domain=-200:400,samples=100]{2*sin(x+180)-1};
\draw(axis cs:290,-2)node[font=\small,fill=white]{$y=2\sin(x+\pi)-1$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
...
\ابتدا{جواب}
\wf{\unexpanded{
\عددی{A=-\tfrac{2}{\pi}, B=4,C=0,D=\tfrac{1}{\pi}}؛ \\
\begin{minipage}{0.45\textwidth}
\begin{tikzpicture}
\begin{axis}[clip=false,tiny,axis lines=middle,width=5cm,ytick={-0.318,0.955},yticklabels={$-\tfrac{1}{\pi}$,$\tfrac{3}{\pi}$},xlabel={$t$},ylabel={$y$},ymin=-0.5,ymax=1.2,xlabel style={at={(current axis.right of origin)},anchor=west},ylabel style={at={(current axis.above origin)},anchor=south},xtick={-1,1,3,5}]
\addplot[domain=-2:6,samples=100]{-2/pi*sin(90*x)+1/pi};
\draw(axis cs:3,0.5)node[font=\small,fill=white]{$y=\tfrac{2}{\pi}\sin\tfrac{\pi t}{2}+\tfrac{1}{\pi}$};
\end{axis}
\end{tikzpicture}
\end{minipage}
}}
