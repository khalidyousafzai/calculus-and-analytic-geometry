...
\draw[-latex](0,0)--({fx(120)},{fy(120)})coordinate(ka)node[pos=0.75,below,yshift=-0.5ex]{$\kvec{r}$}node[circ]{}node[right]{$N(x,y)$};
...
\draw[-latex](ka)--++(120:1)node[right]{$\kvec{T}$};
\end{tikzpicture}
...
\label{شکل_سمتی_تفاعل_در_پیچیدہ}
\end{minipage}
\end{figure}
...
\انتہا{مثال}
%====================
\ابتدا{مثال}\ترچھا{دائرہ کا در پیچیدہ} (شکل \حوالہ{شکل_سمتی_تفاعل_در_پیچیدہ})\\
درج ذیل پیچ دار منحنی کا اکائی مماسی سمتیہ تلاش کریں۔
\begin{align*}
