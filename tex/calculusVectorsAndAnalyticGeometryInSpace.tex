...
\centering
%	9x^2+4y^2+2z^2=36
\begin{tikzpicture}[declare function={fx(\r,\t)=2*\r*cos(t);fy(\r,\t)=3*\r*sin(\t);fz(\r,\t)=4*sqrt(1-(\r)^2);g(\x)=sqrt(18-2*(\x)^2);h(\x)=3/sqrt(2)*sqrt(4-(\x)^2);}]
\pgfmathsetmacro{\ra}{1}
\pgfmathsetmacro{\ta}{0}
\pgfmathsetmacro{\tb}{90}
\pgfmathsetmacro{\tc}{180}
...
%\addplot3[z buffer=sort,surf,domain=0:\ra,domain y=0:360,variable=\r,variable y=\t]({fx(r,t)},{fy(r,t)},{-fz(r,t)});   %zabardast
\addplot3[domain=0:\ra,domain y=0:360,variable=\r,variable y=\t]({fx(\ra,t)},{fy(\ra,t)},{fz(\ra,t)}); 
\addplot3[samples y=0,domain=-3:3]({0},{\x},{g(\x)});
\addplot3[samples y=0,domain=-3:3]({0},{\x},{-g(\x)});
\addplot3[samples y=0,domain=-3:3]({\x},{0},{h(\x)});
\addplot3[samples y=0,domain=-3:3]({\x},{0},{-h(\x)});
...
%====================
\ابتدا{سوال}
$\frac{y^2}{4}+\frac{z^2}{9}-\frac{x^2}{9}=1$
...
%====================
\ابتدا{سوال}
محور \عددی{z} کے عمودی سطحیں   ترخیمی  سطح کے دونوں سروں سے   برابر حصے کاٹ کر دکھایا گیا ا  ڈھول   پیدا کرتی ہیں۔ محور \عددی{} کے قائمہ عمودی تراش دائری ہیں۔  ڈھول کا قد \عددی{2h}،  وسطی رداس \عددی{R} اور سروں کے رداس \عددی{r} ہیں۔ ڈھول کے حجم کا کلیہ تلاش کریں۔ اب دو باتوں  کی تصدیق کریں۔  کیا  ڈھول کے اطراف سیدھا کرنے سے آپ کا کلیہ، قد \عددی{2h} اور رداس \عددی{R} کے  نلکی کا حجم دیتا ہے؟  کیا \عددی{r=0} اور \عددی{h=R}  کی صورت میں، جب   ڈھول    کی شکل ایک کرہ مانند ہو گی، آپ کا کلیہ کرہ کا حجم دیتا ہے؟
\انتہا{سوال}
%=====================
\ابتدا{سوال}
...
\frac{x^2}{a^2}+\frac{y^2}{b^2}=\frac{z}{c}
\end{align*}
سے ایک حصہ کاٹتا ہے۔دکھائیں کہ اس حصے کا حجم ، اس حصے کے قاعدہ کا نصف ضرب قد کے برابر ہو گا۔ (شکل \حوالہ{شکل_مثال_سمتیہ_ترخیمی_سطح} میں \عددی{h=c} کے لئے یہ حصہ دکھایا گیا ہے۔)
\انتہا{سوال}
%===================
\ابتدا{سوال}
