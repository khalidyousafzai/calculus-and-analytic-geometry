...
\حصہ{دوہرا تکملات}
ہم  مستوی \عددی{xy} میں محدود  خطہ  پر استمراری تفاعل \عددی{f(x,y)}  کا تکمل حاصل کرنا سکھاتے ہیں۔یہاں متعارف کیے جانے والا  دوہرا  (دو گنا) تکمل اور باب \حوالہ{باب_تکمل} میں متعارف کردہ  ایک گنا تکمل میں بہت ساری یکساں خوبیاں پائی جاتی ہیں۔ ہر دوہرا  تکمل کی قیمت  ایک گنا تکمل کی ترکیب سے مراحل میں حاصل کی  جا سکتی ہے۔
...
\end{tikzpicture}
\caption{
دوہرا تکملات بھی ایک گنا تکملات کی طرح  مجموعیت دائرہ کار کی خاصیت رکھتے ہیں۔
}
...
ایک گنا تکملات کی طرح، دوہرا تکملات کے ایسا الجبرائی خواص پائے جاتے ہیں جو حساب اور عملی استعمال کے لیے کارآمد ثابت ہوتے ہیں۔
\begin{enumerate}[a.]
\item\quad
...
یہ خواص ایک گنا تکملات  کے خواص کی طرح ہیں (حصہ \حوالہ{حصہ_تکمل_خصوصیات_رقبہ_اوسط_قیمت_مسئلہ})۔ ان کے علاوہ  درج ذیل مجموعیت کا خواص بھی پایا جاتا ہے
\item\quad
...
\end{tikzpicture}
\caption{
رقبہ عمودی تراش \عددی{S(x)} حاصل کرنے کے لیے ہم \عددی{x} کو مستقل ٹھراتے  ہوئے \عددی{y} کے لحاظ سے  تکمل لیتے ہیں۔
}
...
\end{tikzpicture}
\caption{
رقبہ عمودی تراش \عددی{S(y)} حاصل کرنے کے لیے ہم \عددی{y} کو مستقل ٹھراتے  ہوئے \عددی{x} کے لحاظ سے  تکمل لیتے ہیں۔
}
...
\end{align}
ہو گا  جہاں \عددی{x} پر رقبہ عمودی تراش \عددی{S(x)} ہے۔ہم    \عددی{x}  کی ہر قیمت کے لیے    درج ذیل تکمل سے  \عددی{S(x)}  معلوم کر سکتے ہیں
...
دائیں ہاتھ الجبرائی فقرہ، جسے  \اصطلاح{بارہا تکمل}\فرہنگ{تکمل!بارہا}\حاشیہب{repeated integral}\فرہنگ{integral!repeated} یا  \اصطلاح{اعادہ تکمل}\فرہنگ{تکمل!اعادہ}\حاشیہب{iterated integral}\فرہنگ{integral!iterated} کہتے ہیں، کہتا ہے  کہ حجم تلاش کرنے کی خاطر، پہلے  \عددی{x} کو مستقل  ٹھراتے  ہوئے \عددی{y} کے لحاظ سے \عددی{4-x-y} کا   تکمل \عددی{y=0} تا \عددی{y=1}   لیں اور اس کے بعد  \عددی{y} کو مستقل ٹھراتے  ہوئے، \عددی{x} کے لحاظ سے حاصل نتیجہ کا تکمل   \عددی{x=0} تا \عددی{x=2}  لیں۔
 ...
 اگر ہم  محور  \عددی{y}  کے عمودی ٹکیاں لیتے تب نتیجہ کیا ہوتا (شکل \حوالہ{شکل_بالکثرت_فوبینی_وائے_مستقل})؟    ایسی صورت میں   ایک علامتی رقبہ  عمودی تراش  ، \عددی{y} کا تفاعل ہو گا:
\begin{align}
...
مسئلہ فوبینی مزید کہتا ہے کہ  دوہرا تکمل کی قیمت حاصل کرتے ہوئے ہم بارہا تکمل کسی بھی ترتیب سے حل کر سکتے ہیں، جو بہت کارآمد ثابت ہوتا ہے (جیسا ہم   مثال \حوالہ{مثال_بالکثرت_غیر_بنیادی_تفاعل} میں دیکھتے ہیں)۔بالخصوص حجم کی تلاش میں ہم محور \عددی{x} یا محور \عددی{y} کے عمودی سطحیں لے کر ٹکیاں کاٹ سکتے ہیں۔
...
\end{tikzpicture}
\caption{
مستطیل خطہ کی مجموعیت کی خاصیت ان غیر مستطیل  خطوں کے لیے بھی کارآمد ہے جن کی پوری  سرحد استمراری منحنیات سے بنی ہو۔
}
...
\end{tikzpicture}
\caption{
سایہ دار انتصابی ٹکیہ  کا رقبہ \عددی{S(x)=\int_{g_1(x)}^{g_2(x)}f(x,y)\dif y} ہو گا۔ اس ٹھوس  جسم کا حجم تلاش کرنے کے لیے ہم  \عددی{x=a} سے \عددی{x=b} تک \عددی{S(x)} کا تکمل لیں گے۔ 
}
\label{شکل_بالکثرت_انتصابی_ٹکیہ_کا_رقبہ}
\end{figure}


اگر شکل \حوالہ{شکل_بالکثرت_انتصابی_ٹکیہ_کا_رقبہ}  میں  مستوی \عددی{xy} میں دکھائے گئے خطہ کی طرح \عددی{R}ہو  اور حجم کی   \قول{بالائی}   حد \عددی{y=g_2(x)}،  \قول{زیریں}  حد \عددی{y=g_1(x)}، اور اطراف کی حدود  خط \عددی{x=a} اور خط  \عددی{x=b} ہوں تب  ہم حجم \عددی{H}  کو  ٹکیوں کی ترکیب سے حاصل کر سکتے ہیں۔ ہم پہلے رقبہ عمودی تراش تلاش کرتے ہیں
\begin{align*}
...
اسی طرح اگر  شکل \حوالہ{شکل_بالکثرت_انتصابی_ٹکیہ_ٹھوس_حجم} میں  دکھائے گئے خطہ کی طرح \عددی{R} ہو اور حجم کی حدود \عددی{x=h_2(y)}، \عددی{x=h_1(y)} اور خط \عددی{y=c} اور \عددی{y=d} ہوں تب  ٹکیوں کی ترکیب سے بارہا تکمل سے حجم تلاش کیا جا سکتا ہے:
...
\end{align*}
\حل
ہم دیکھتے ہیں (شکل \حوالہ{شکل_مثال_بالکثرت_فوبینی_مثلث}-ا)  کہ  \عددی{0} اور \عددی{1} تک کسی بھی  \عددی{x} کے لیے \عددی{y} کی قیمت \عددی{y=0} تا \عددی{y=x} ہو گی (شکل \حوالہ{شکل_مثال_بالکثرت_فوبینی_مثلث}-ب)۔ یوں درج ذیل ہو گا۔
\begin{align*}
...
(ا) \quad
   خطہ \عددی{R} پر \عددی{\iint_R f(x,y)\dif S} کی قیمت حاصل کرتے ہوئے  پہلے \عددی{y} اور بعد میں \عددی{x} کے لحاظ سے تکمل لینے کے لیے درج ذیل اقدام کریں۔
\begin{enumerate}[1.]
\item
\ترچھا{خاکہ:}\quad
...
(ب) \quad 
اسی دوہرا تکمل کو   بطور  بارہا تکمل حل کرتے ہوئے، ترتیب الٹ کرنے سے، انتصابی لکیروں کے بجائے  افقی لکیریں استعمال کریں (شکل \حوالہ{شکل_بالکثرت_بار_بار_تکمل_الٹ_ترتیب})۔ تکمل درج ذیل ہو گا۔
\begin{align*}
...
تکمل کی ترتیب الٹ کرتے ہوئے ہم اس خطہ  پر افقی لکیریں کھینچتے ہیں۔ یہ لکیریں اس خطہ میں \عددی{x=\tfrac{y}{2}} پر داخلی ہوتی ہیں اور \عددی{x=\sqrt{y}} پر اس سے خارج ہوتی ہیں۔ ان تمام افقی لکیریں کو شامل کرنے کے لیے ہمیں \عددی{y=0} سے \عددی{y=4} تک لینا ہو گا (شکل \حوالہ{شکل_مثال_بالکثرت_منحنیات_بیچ_خطہ_دوہرا_تکمل}-ب)۔ یوں  متبادل تکمل درج ذیل ہو گا۔
\begin{align*}
...
%====================
\ابتدا{سوال}
چوکور \عددی{1\le x\le 2,\, 1\le y\le 2} پر تفاعل \عددی{f(x,y)=\tfrac{1}{xy}}  کا تکمل۔
\انتہا{سوال}
%==================
\ابتدا{سوال}
...
جہاں \عددی{R} چوکور \عددی{\abs{x}+\abs{y}=1} کا اندرونی خطہ  ہے۔
\انتہا{سوال}
%================
\ابتدا{جواب}
\wf{\unexpanded{
...
%====================
\ابتدا{سوال}
چوکور ستون   \عددی{\abs{x}+\abs{y}\le 1} سے مستویات  \عددی{z=0} اور \عددی{3x+z=3}  جس ٹھوس جسم کو کاٹتے ہیں اس کا حجم تلاش کریں۔
\انتہا{سوال}
%==============
\ابتدا{سوال}
...
%===================
\ابتدا{سوال}
ایک ٹھوس (غیر دائری) قائمہ بیلن کا قاعدہ مستوی \عددی{xy} ہے جبکہ اس کی بالائی سرحد قطع مکافی سطح  \عددی{z=x^2+y^2} ہے۔ اس بیلن کا حجم
\begin{align*}
...
%================
\ابتدا{سوال}
مستوی \عددی{xy} میں کون سا خطہ \عددی{R} درج ذیل تکمل کی قیمت کو زیادہ سے زیادہ بناتا ہے؟
\begin{align*}
...
%=============
\ابتدا{سوال}
مستوی \عددی{xy} میں کون سا خطہ \عددی{R} درج ذیل تکمل کی قیمت کو کم سے کم  بناتا ہے؟
\begin{align*}
...
\end{tikzpicture}
\caption{
ایک خطہ کے رقبے کی تلاش میں پہلا قدم خطے کے اندرون کی خانہ بندی ہے۔
}
...
\caption{}
\end{subfigure}
\caption{(ا) اگر ہم پہلے \عددی{x} کے لحاظ سے تکمل لیں تب رقبے کے حصول کے لیے دو  تکملات کا مجموعہ درکار ہو گا۔ (ب) البتہ پہلے \عددی{y} کے لحاظ سے تکمل لیتے ہوئے  صرف ایک تکمل سے حاصل ہو گا۔}
...
باریک چادروں کی کمیت اور معیار اثر تلاش کرنے کے لیے ہم  باب \حوالہ{باب_تکمل_کا_استعمال} کے کلیات کی طرح کلیات استعمال کرتے ہیں۔ فرق صرف اتنا ہے کہ دوہرا تکمل کی وجہ سے اب ہم زیادہ  اشکال اور  کثافتی تفاعل کو  عمل میں لا  سکتے ہیں۔   جدول میں ان کلیات درج ذیل ہیں۔
...
معیار اثر اول \عددی{M_x} اور   \عددی{M_y} اور معیار اثر دوم  (جمودی معیار اثر) \عددی{I_x} اور \عددی{I_y}  میں  ریاضیاتی فرق یہ ہے کہ معیار اثر دور  \قول{بیرم کے بازوؤں} کے فاصلوں،      \عددی{x} اور \عددی{y}،      کا مربع لیتا ہے۔ 
...
\caption{}
\end{subfigure}
\caption{دونوں شہتیر کا رقبہ عمودی تراش ایک سا  ہے لیکن شہتیر -ا  کا جمودی معیار اثر زیادہ ہے لہٰذا  شہتیر- ا  زیادہ اکڑ ہو گا۔}
...
\حل
ہم اس خطہ کا خاکہ بنا کر (شکل \حوالہ{شکل_مثال_بالکثرت_تکون_رداس_دوار})   اس پر اتنی معلومات درج کرتے ہیں کہ تکمل کی حدیں  جان سکیں۔
...
\حل
ہم خطے کا خاکہ بنا کر تکمل کی حدیں جانتے ہیں (شکل \حوالہ{شکل_مثال_بالکثرت_مربع_خطی_بیچ})۔ اس کے بعد \عددی{\delta=1} لے کر آگے بڑھتے ہیں۔
\begin{align*}
...
%====================
\ابتدا{سوال}
کیا چوکور \عددی{0\le x\le 1,\, 0\le y\le 1} یا ربع اول میں دائرہ \عددی{x^2+y^2=1}  میں \عددی{f(x,y)=xy}  کی اوسط قیمت زیادہ ہو گی؟ ان دونوں خطوں میں اوسط کی قیمت تلاش کریں۔
\انتہا{سوال}
%================
\ابتدا{سوال}
چوکور \عددی{0\le x\le 2,\, 0\le y\le 2} میں قطع مکافی \عددی{z=x^2+y^2} کا  اوسط قد تلاش کریں۔
\انتہا{سوال}
%===========
\ابتدا{جواب}
\wf{\unexpanded{
...
%====================
\ابتدا{سوال}
چوکور \عددی{\ln 2\le x\le 2\ln 2,\, \ln 2\le y\le 2\ln 2} میں \عددی{f(x,y)=\tfrac{1}{xy}} کی اوسط قیمت تلاش کریں۔
\انتہا{سوال}
%================
\موٹا{مستقل کثافت}\\
\ابتدا{سوال}
...
%====================
\ابتدا{سوال}
دائرہ \عددی{x^2+y^2=4} کے بیچ کثافت \عددی{\delta=1}  کی باریک چادر کے  محور \عددی{x} کے لحاظ سے جمودی معیار اثر تلاش کریں۔ اس نتیجہ کو استعمال کرتے ہوئے اس خطہ کی \عددی{I_y} اور \عددی{I_0} دریافت کریں۔
\انتہا{سوال}
%==================
\ابتدا{سوال}
...
\موٹا{نظریہ اور مثالیں}\\
\ابتدا{سوال}
مستوی \عددی{xy} میں جراثیم   کی تعدادی کثافت \عددی{f(x,y)=\tfrac{10000e^y}{1+\abs{x}/2}} ہے جہاں \عددی{x} اور \عددی{y} کی ناپ سنٹی میٹر میں ہے۔ مستطیل \عددی{-5\le x\le 5,\, -2\le y\le 0} میں جراثیم کی کُل تعداد تلاش کریں۔
\انتہا{سوال}
%=================
\ابتدا{جواب}
\wf{\unexpanded{
...
%====================
\ابتدا{سوال}
سطح زمین پر  کثافت آبادی \عددی{f(x,y)=100(y+1)} ہے جہاں \عددی{x} اور \عددی{y} کلومیٹر میں ہیں۔منحنیات \عددی{x=y^2} اور \عددی{x=2y-y^2} کے بیچ کُل آبادی کتنی ہو گی؟
\انتہا{سوال}
%==============
\ابتدا{سوال}
...
\end{align}
مساوات \حوالہ{مساوات_بالکثرت_کلیہ_پاپس} کو \اصطلاح{کلیہ پاپس}\فرہنگ{کلیہ!پاپس}\حاشیہب{Pappus's formula}\فرہنگ{Pappus!formula} کہتے ہیں۔ایک دوسرے کو نہ ڈھانپتی ہوئی دو سے زیادہ (لیکن متناہی تعداد کی)  چادروں کے لیے درج ذیل کلیہ ہو گا۔
...
%==================
\ابتدا{سوال}
ریاضی (الکراجی)  ماخوذ اور مساوات \حوالہ{مساوات_بالکثرت_کلیہ_پاپس} استعمال کرتے ہوئے دکھائیں کہ کسی بھی عدد صحیح \عددی{n>2} کے لیے  مساوات \حوالہ{مساوات_بالکثرت_عمومی_کلیہ_پاپس} مطمئن ہو گا۔
\انتہا{سوال}
%===================
...
ایک مساوی الساقین  مثلث  \عددی{T} کا قاعدہ \عددی{2a} اور قد \عددی{h} ہے۔ اس کا قاعدہ ، رداس \عددی{a} کے  نصف دائرہ \عددی{D}   کے قطر پر   پایا جاتا ہے۔ مثلث دائرہ کے باہر ہے۔  \عددی{T\cup D}  کا وسطانی مرکز (ا)  \عددی{T} اور \عددی{D} کی مشترک سرحد پر  (ب)  \عددی{T} کے اندر ہونے کے لیے \عددی{a} اور \عددی{h} کا تعلق دریافت کریں۔
\انتہا{سوال}
%==========
\ابتدا{جواب}
\wf{\unexpanded{
...
\انتہا{جواب}
%====================
\ابتدا{سوال}\شناخت{سوال_بالکثرت_چوکور_قاعدہ}
ایک مساوی الساقین مثلث  \عددی{T} جس کا قد \عددی{h} ہے کا قاعدہ  چوکور \عددی{Q}  کا ایک ضلع ہے۔ چوکور کے   ضلع کی لمبائی  \عددی{s}  ہے۔ (چوکور اور مثلث ایک دوسرے کو نہیں ڈھانپتے ہیں۔) \عددی{T\cup Q}  کا  وسطانی مرکز مثلث کے قاعدہ پر رکھنے کی خاطر \عددی{h} کا  \عددی{s} کے ساتھ کیا  تعلق   گا؟  اپنے جواب کا موازنہ سوال \حوالہ{سوال_بالکثرت_مثلث_دائرہ} کے جواب کے ساتھ کریں۔
\انتہا{سوال}
%==========
\انتہا{سوالات}
...
مستوی \عددی{xy} میں دوہرا تکمل کا ذکر کرتے ہوئے  ہم نے خطہ \عددی{R} کو  مستطیلی ٹکڑوں میں اس طرح   کاٹا کہ مستطیل کے اضلاع محددی محوروں کے متوازی ہوں۔اس طرح   ان مستطیلوں کے اضلاع  مستقل \عددی{x} اور یا مستقل \عددی{y} لکھے جا سکتے  ہیں۔ کارتیسی محدد میں مستطیل قدرتی صورت ہے۔ قطبی محددی نظام میں \قول{قطبی مستطیل} قدرتی صورت ہے جس کے اضلاع مستقل \عددی{r} اور مستقل \عددی{\theta} لکھے جا سکتے ہیں۔

فرض کریں تفاعل  \عددی{f(r,\theta)} خطہ \عددی{R} پر معین ہے  جس  کے سرحد شعاع \عددی{\theta=\alpha} اور \عددی{\theta=\beta} اور استمراری منحنیات \عددی{r=g_1(\theta)} اور \عددی{r=g_2(\theta)}  ہیں۔مزید \عددی{\alpha} اور \عددی{\beta} کے بیچ ہر قیمت کے لیے \عددی{0\le g_1(\theta)\le g_2(\theta)\le a} ہے۔ یوں  \عددی{R} پنکھا نما  خطہ \عددی{Q} میں، جس کو عدم مساوات \عددی{0\le r\le a,\, \alpha\le\theta\le\beta} ظاہر کرتی ہیں، پایا جائے گا (شکل \حوالہ{مساوات_بالکثرت_قطبی_مستطیل_خانے_الف})۔
...
\end{tikzpicture}
\caption{
سایہ دار خطے کا رقبہ \عددی{\Delta S_k} حاصل کرنے کے لیے بڑے خطے سے چھوٹے خطے  کا رقبہ منفی کریں۔
}
...
\end{align*}
یہ شعاع اور قوسین \عددی{Q} کو  \قول{قطبی مستطیلوں} میں تقسیم کرتے ہیں۔

ہم ان قطبی مستطیلوں کو \عددی{1} تا \عددی{n}  کی   شمار سے ظاہر کرتے ہیں جو مکمل طور پر  \عددی{R} کے اندر پائے جاتے ہوں اور ان کے رقبوں کو \عددی{\Delta S_1}، \عددی{\Delta S_2}، \نقطے، \عددی{\Delta S_n} سے ظاہر کرتے ہیں۔ شمار کرنے کی ترتیب غیر ضروری ہے۔  ہم   \عددی{\Delta S_k}  رقبے کی قطبی مستطیل  کے  مرکز کو  \عددی{(r_k,\theta_k)} سے ظاہر کرتے ہیں۔ قطبی مستطیل کے مرکز سے مراد وہ نقطہ ہے جو  دونوں دائری قوسین کے اوسط  رداس کی  قوس  اور  اس شعاع پر پایا جاتا ہو جو دونوں قوسین کو درمیان سے کاٹتی ہو۔  ہم اب  درج ذیل مجموعہ لیتے ہیں۔
...
مسئلہ فوبینی کی ایک صورت کہتی ہے کہ اس مجموعہ  کی حد  \عددی{r} اور \عددی{\theta} کے لحاظ سے  درج ذیل  بارہا تکمل  دے گا۔
\begin{align}
...
کارتیسی محدد میں تکمل کی حدیں تلاش کرنے کا طریقہ کار  قطبی محدد کے لیے  بھی کارآمد ہے۔
...
قطبی محدد میں خطہ \عددی{R} پر \عددی{\iint_Rf(r,\theta)\dif S} کی قیمت حاصل کرنے کے لیے    پہلے \عددی{r} کے لحاظ سے اور بعد میں \عددی{\theta} کے لحاظ سے تکمل لیتے ہوئے ہمیں  درج ذیل اقدام کرنے ہوں گے۔
\begin{enumerate}[1.]
\item
\ترچھا{خاکہ:}\quad
...
\end{align}
یہ  باب \حوالہ{باب_تکمل} میں ترکیب بدل کی طرح ہے البتہ یہاں ایک کے بجائے دو متغیرات ہیں۔دھیان رہے کہ \عددی{\dif x\dif y} کی جگہ \عددی{\dif r\dif\theta} نہیں بلکہ \عددی{r\dif r\dif \theta} پر کیا جاتا ہے۔ اس کی وجہ حصہ \حوالہ{حصہ_دہرا_تکملات_بالکثرت_میں_بدل}  پیش کی جائے گی۔
...
%==================
\ابتدا{سوال}
دائرہ \عددی{x^2+y^2=a^2} کے اندر باریک  دائرہ قرص  کی کثافت \عددی{\delta(x,y)=k(x^2+y^2)} ہے جہاں \عددی{k} ایک مستقل ہے۔ اس قرص  کے  محور \عددی{x} کے لحاظ سے جمودی معیار اثر اور مبدا کے لحاظ سے قطبی معیار اثر تلاش کریں۔
\انتہا{سوال}
%============
\ابتدا{سوال}
...
\item
جزو-ا میں خطہ کی ہر سرحد کی کارتیسی مساوات کو \عددی{r} اور \عددی{\theta} کے لیے حل کرتے  ان کی قطبی مساوات تلاش کریں۔
\item
...
ہم تہرا تکملات کی مدد سے تین بُعدی اجسام کے حجم، کمیت اور معیار اثر  اور تین متغیری تفاعل کی اوسط قیمت معلوم کرتے ہیں۔  باب \حوالہ{باب_سمتی_میدان_میں_تکمل} میں ہم دیکھیں گے کہ سمتی میدان   اور  حرکت سیال  کے مطالعہ میں ہمیں ان تکملات سے کیسا واسطہ پڑتا ہے۔
...
\end{align}
اگر \عددی{F} استمراری ہو اور \عددی{D} کی تحدیدی سطح ہموار سطحوں پر مشتمل ہو جو ایک دوسرے کے ساتھ استمراری منحنیات میں جڑتے ہوں، تب جوں جوں \عددی{\Delta x_k}، \عددی{\Delta y_k} اور \عددی{\Delta z_k} صفر کے قریب پہنچتے ہوں  توں توں  مجموعات \عددی{J_n}   ایک حد تک پہنچتی ہیں:
\begin{align}
\lim_{n\to \infty}J_n=\iiint\limits_D F(x,y,z)\dif H
\end{align} 
ہم اس حد کو\اصطلاح{ \عددی{D} پر \عددی{F} کا تہرا تکمل}\فرہنگ{تکمل!تہرا}\حاشیہب{triple integral}\فرہنگ{integral!triple} کہتے ہیں۔ یہ حد چند غیر استمراری تفاعل کے لیے بھی موجود ہے۔
\begin{figure}
\centering
\begin{subfigure}{0.45\textwidth}
\centering
...
جوں جوں \عددی{\Delta x_k}، \عددی{\Delta y_k} اور \عددی{\Delta z_k} صفر تک پہنچتے ہیں  توں توں  \عددی{\Delta H_k}  جسامت میں چھوٹے  اور تعداد میں زیادہ ہوتے جاتے ہیں اور \عددی{D} کے زیادہ حصہ کو بھرتے ہیں۔اسی لیے ہم \عددی{D} کے حجم  کی تعریف درج ذیل لیتے ہیں۔
\begin{align*}
...
ہم تہرا تکمل کی تعریف سے اس کی قیمت شاذ و نادر حاصل کرتے ہیں۔ اس کے بجائے ہم مسئلہ فوبینی کی  تین بُعدی روپ استعمال کرتے ہوئے تین بار ایک گنا تکملات سے اس کی قیمت معلوم کرتے ہیں۔ دہرا تکمل کی طرح، تکمل کی  حدیں  معلوم کرنے کا جیومیٹریائی طریقہ کار پایا جاتا  ہے۔
 \begin{figure}
\centering
\begin{subfigure}{0.30\textwidth}
\centering
...
خطہ \عددی{R} میں  علامتی نقطہ \عددی{(x,y)} سے محور \عددی{z} کے متوازی  لکیر \عددی{M} کھینچیں۔  بڑھتے  \عددی{z} رک چلتے ہوئے،  یہ لکیر \عددی{z=f_1(x,y)} پر \عددی{D} میں داخل ہو گی اور  \عددی{z=f_2(x,y)} پر \عددی{D} سے خارج ہو گی۔یہی تکمل کی  \عددی{z} حدیں ہیں (شکل \حوالہ{شکل_بالکثرت_تہرا_تکمل_کی_حدوں_کی_تلاش}-ب)۔
\item
\ترچھا{تکمل کی \عددی{y} حدیں:}\quad
 نقطہ \عددی{(x,y)} سے گزرتی ہوئی  محور \عددی{y} کے متوازی  لکیر \عددی{L} کھینچیں۔ بڑھتے \عددی{y} رخ چلتے ہوئے  یہ لکیر \عددی{R} میں \عددی{y=g_1(x)} پر داخل اور \عددی{y=g_2(x)} پر خارج ہو گی۔ یہی تکمل کی \عددی{y} حدیں ہیں (شکل \حوالہ{شکل_بالکثرت_تہرا_تکمل_کی_حدوں_کی_تلاش}-ج)۔
\item
\ترچھا{تکمل کی \عددی{x} حدیں:}\quad
...
\حل
ہم \عددی{F(x,y,z)=1} لیتے ہوئے حجم کے لیے درج ذیل تکمل لکھتے ہیں۔
\begin{align*}
...
اگلی مثال میں ہم  مستوی \عددی{xy} کے بجائے مستوی \عددی{xz} میں \عددی{D} کا سایہ لیتے ہیں۔
...
\item
\ترچھا{تکمل کی \عددی{x} حدیں:}\quad
جب \عددی{L} خطہ \عددی{R} میں   گزرتا ہے \عددی{x} کی قیمت \عددی{x=0} سے \عددی{x=1} تک   ہوتی ہے۔
\end{enumerate}
یوں تکمل درج ذیل ہو گا۔
\begin{align*}
...
\end{figure}
جیسا ہم جانتے ہیں، دہرا تکمل کا حصول عموماً (لیکن ضروری نہیں)    ایک گنا تکملات کو دو مختلف  ترتیب سے حاصل کر کے حاصل کرنا ممکن ہوتا ہے۔ تہرا تکمل کے لیے اس طرح کے چھ  ترتیب ممکن ہو سکتے ہیں۔
...
%=================
\ابتدا{سوال}
ٹھوس ترخیمی بیلن \عددی{x^2+4y^2\le 4}  سے مستوی \عددی{xy} اور مستوی \عددی{z=x+2} جو خطہ کاٹتے ہیں۔
\انتہا{سوال}
%=================
\ابتدا{جواب}
\wf{\unexpanded{
...
\موٹا{نظریہ اور مثالیں}\\
\ابتدا{سوال}
درج ذیل کو \عددی{ a} کے لیے حل کریں۔
\begin{align*}
...
%=================
\ابتدا{سوال}
ترخیمی سطح \عددی{x^2+\tfrac{y^2}{4}+\tfrac{z^2}{c^2}=1} کا حجم  \عددی{c} کی کس قیمت کے لیے \عددی{8\pi} ہو گا؟
\انتہا{سوال}
%===================
\ابتدا{سوال}
فضا میں  کون سا دائرہ کار \عددی{D} درج ذیل تکمل کی قیمت کو کم سے کم بناتا ہے؟ اپنے جواب کی وجہ پیش کریں۔
\begin{align*}
...
%=============
\ابتدا{سوال}
فضا میں کون سا دائرہ کار \عددی{D} درج ذیل تکمل کی قیمت کو زیادہ سے زیادہ بناتا ہے؟ اپنے جواب کی وجہ پیش کریں۔
\begin{align*}
...
اس حصہ میں تین بُعدی اجسام کی  کمیت اور معیار اثر کا حصول کارتیسی محدد میں سکھایا جائے گا۔ یہ کلیات دو بُعدی  اجسام کے کلیات کی طرح ہیں۔ کروی اور نلکی محدد میں حساب کرنا  حصہ \حوالہ{حصہ_سمتی_تکمل_نلکی_کروی_تہرا_تکمل} میں دکھایا جائے گا۔

\جزوحصہء{کمیت اور معیار اثر}
فضا میں خطہ \عددی{D} میں پائے جانے والے ایک جسم کی  کمیتی کثافت \عددی{\delta(x,y,z)} ہے۔ خطہ \عددی{D} پر \عددی{\delta} کا تکمل اس جسم کی کمیت دے گا۔ یہ دیکھنے کی خاطر کہ ایسا کیوں  کر ہو گا  ہم  اس جسم کو \عددی{n} ٹکڑوں  میں تقسیم کرتے ہیں (شکل \حوالہ{شکل_بالکثرت_ٹھوس_جسم_کے_ٹکڑے})۔ جسم کی کمیت درج ذیل حد ہو گی۔
...
\حل
تشاکلی کی وجہ سے \عددی{\bar{x}=\bar{y}=0} ہو گا۔ ہمیں \عددی{\bar{z}}  معلوم  کرنے کے لیے پہلے درج ذیل دریافت  کرنے ہوں گے۔
\begin{align*}
...
جب جسم کی کثافت  اٹل ہو (جیسا مثال \حوالہ{مثال_بالکثرت_تین_بعدی_ٹھوس_جسم_الف}  اور مثال \حوالہ{مثال_بالکثرت_تین_بعدی_ٹھوس_جسم_ب} میں تھا)، تب (دو  بُعدی اجسام کی طرح)   مرکز کمیت اس جسم کا \اصطلاح{وسطانی مرکز}\فرہنگ{مرکز!وسطانی}\حاشیہب{centroid}\فرہنگ{centroid} ہو گا۔

\حصہء{سوالات}\شناخت{سوالات_کثیر_التکمل_پانچ}
\موٹا{مستقل کثافت}\\
...
%=========================
\ابتدا{سوال}
مستقل کثافت کے ایک ٹھوس \قول{کونڈا}  کی زیریں سرحدی سطح \عددی{z=4y^2}،  بالائی سرحدی سطح \عددی{z=4} اور اطراف  مستویات \عددی{x=1} اور \عددی{x=-1} ہیں۔ اس کی مرکز کمیت  اور تینوں محوروں کے لحاظ سے جمودی معیار اثر تلاش کریں۔
\انتہا{سوال}
%============================
...
\begin{enumerate}[a.]
\item
    مکمل بھرے ہوئے برتن سے   سیال  کو مستوی \عددی{xy} میں منتقل کرنے کے لیے مستقل  تجاذب  \عددی{g}  کتنا  کام کرے گا؟   (اشارہ: برتن میں سیال  کو چھوٹے چھوٹے حجم کے ٹکڑوں \عددی{\Delta H_k}  میں تقسیم کرتے ہوئے  ہر ٹکڑے کو منتقل کرنے کے لیے درکار کام دریافت کریں۔ ان تمام کا مجموعہ پورے سیال کو منتقل کرنے کا کام ہو گا۔ یہ مجموعہ، حد کی صورت میں، تہرا تکمل دے گا جس کی قیمت آپ کو معلوم کرنی ہو گی۔)
\item
مکمل بھرے ہوئے برتن میں  سیال کے مرکز کمیت کو مستوی \عددی{xy} میں منتقل کرنے کے لیے مستقل تجاذب \عددی{g} کتنا کام کرے گا؟
\end{enumerate}
...
\موٹا{مسئلہ متوازی محور}\شناخت{سوال_کثیر_التکمل_مسئلہ_متوازی_محور}\\
مسئلہ متوازی محور (حصہ \حوالہ{حصہ_بالکثرت_رقبات_معیار_اثر_مرکز_کمیت} کے سوالات دیکھیں)   دو بُعدی صورت  کے ساتھ ساتھ تین بُعدی صورت  کے لیے بھی کارآمد ہے۔فرض کریں ایک جسم جس کی کمیت \عددی{m} ہو کے مرکز کمیت سے خط \عددی{L_{c,m}} گزرتا ہو  جس کے متوازی  \عددی{h} فاصلہ پر خط \عددی{L} پایا جاتا ہو ۔ مسئلہ متوازی محور کہتا ہے کہ \عددی{L_{c,m}} اور \عددی{L} کے لحاظ سے اس جسم کے جمودی معیار اثر درج ذیل کلیہ کو مطمئن کرتے ہیں۔
\begin{align}\label{مساوات_بالکثرت_مسئلہ_متوازی_محور_الف}
I_L=I_{c,m}+mh^2
\end{align}
دو بُعدی  صورت کی طرح اگر ہمیں ایک جمودی معیار اثر، فاصلہ \عددی{h} اور جسم کی کمیت \عددی{m} معلوم ہو تب   ہم اس  مسئلہ کی  مدد سے  دوسرا جمودی معیار اثر با آسانی  دریافت کر سکتے ہیں۔
...
کلیہ پاپس  (حصہ \حوالہ{حصہ_بالکثرت_رقبات_معیار_اثر_مرکز_کمیت} کے سوالات دیکھیں)   دو بُعدی صورت  کے ساتھ ساتھ تین بُعدی صورت  کے لیے بھی کارآمد ہے۔ فرض کریں دو اجسام \عددی{B_1} اور \عددی{B_2} جن کی  کمیتیں   بالترتیب \عددی{m_1} اور \عددی{m_2} ہوں  فضا میں ایک دوسرے کو  نہ ڈھانپتے ہوئے خطوں  میں پائے جاتے ہیں۔ مبدا سے ان اجسام کے مراکز کمیت تک سمتیات بالترتیب \عددی{\kvec{c}_1} اور \عددی{\kvec{c}_2} ہیں۔تب ان کے اشتراک  \عددی{B_1\cup B_2}  کے مرکز کمیت کا تعین گر سمتیہ درج ذیل ہو گا۔
\begin{align}\label{مساوات_بالکثرت_کلیہ_تین_بعدی_کلیہ_پاپس_الف}
\kvec{c}=\frac{m_1\kvec{c}_1+m_2\kvec{c}_2}{m_1+m_2}
\end{align}
پہلے کی طرح، اس کو \اصطلاح{کلیہ پاپس}\فرہنگ{کلیہ!پاپس}\حاشیہب{Pappus's formula}\فرہنگ{Pappus's formula} کہتے ہیں۔ دو بُعدی صورت کی طرح،  \عددی{n} عدد اجسام کے لیے   اس کلیہ کی عمومی روپ درج ذیل ہو گی۔
...
خط \عددی{L}  جھاڑو کی طرح \عددی{R} کو جھاڑتے ہوئے مثبت محور \عددی{x} کے ساتھ \عددی{\phi=0} اور \عددی{\phi=\pi}کے بیچ رہتا ہے۔
\end {enumerate}
یوں تکمل درج ذیل ہو گا۔
\begin{align*}
...
\item
\ترچھا{تکمل کی \عددی{\phi} حدیں:}\quad
لکیر \عددی{L} خطہ \عددی{R} کو جھاڑتے ہوئے  مثبت محور \عددی{x} کے ساتھ زاویہ \عددی{\phi=\alpha} اور \عددی{\phi=\beta}  کے بیچ رہتی ہے (شکل \حوالہ{شکل_بالکثرت_نلکی_محدد_تکمل_کا_حصول}-ج)۔ یہی تکمل کی \عددی{\phi} حدیں ہوں گی۔
\end{enumerate}
یوں تکمل درج ذیل ہو گا۔
\begin{align}
...
\theta&=\frac{\pi}{3}&&\text{\RL{مبدا سے اوپر رخ کھلتا ہوا مخروط جو مثبت محور \عددی{z} کے ساتھ \عددی{\pi/3} زاویہ بناتا ہے}}\\
\phi&=\frac{\pi}{3}&&\text{\RL{نصف چادر جس کا چول محور \عددی{z} ہے اور جو مثبت محور  \عددی{x}  کے ساتھ \عددی{\pi/3} زاویہ بناتا ہے}}
\end{align*}
%
\begin{figure}
\centering
...
\end{align*}
ہم عموماً پہلے \عددی{r} کے لحاظ سے تکمل لیتے ہیں۔ ہم  صرف ان تکملات  پر غور کریں گے  جو محور \عددی{z} کے لحاظ سے اجسام طواف  (یا ان کا حصہ) ہوں اور جن کے \عددی{\theta} اور \عددی{\phi} حدیں مستقل ہوں۔
...
\end{tikzpicture}
\end{subfigure}%
\caption{کروی محدد میں تہرا تکمل کی حدوں کی تلاش۔}
...
\item
\ترچھا{تکمل کی \عددی{r} حدیں:}\quad
مبدا سے ایک لکیر \عددی{M} کھینچیں جو مثبت محور \عددی{z} کے ساتھ زاویہ \عددی{\phi} بناتی ہو۔ ساتھ ہی \عددی{R} پر \عددی{M} کی تظلیل \عددی{L} کا خاکہ بنائیں جو مثبت محور \عددی{x} کے ساتھ زاویہ \عددی{\phi} بنائی گی۔ جیسے جیسے \عددی{r} بڑھے گا \عددی{M} خطہ \عددی{D} میں \عددی{r=g_1(\theta,\phi)} سے داخل اور \عددی{r=g_2(\theta,\phi)} سے خارج ہو گی۔ یہی تکمل کی \عددی{r} حدیں ہوں گی۔
\item
\ترچھا{تکمل کی \عددی{\phi} حدیں:}\quad
کسی بھی مخصوص \عددی{\phi} کے لیے  \عددی{M} مثبت محور \عددی{z} کے ساتھ \عددی{\theta=\theta_1} سے \عددی{\theta=\theta_2} تک زاویہ بنائے گی۔یہی تکمل کی \عددی{\theta} حدیں ہوں گی۔
\item
\ترچھا{تکمل کی \عددی{\phi} حدیں:}\quad
کسی بھی مخصوص \عددی{\phi} کے لیے \عددی{L}   خطہ \عددی{R} پر جھاڑو کی طرح چلتے ہوئے \عددی{\phi=\alpha} سے \عددی{\phi=\beta} تک چلتی ہے۔  یہی تکمل کی \عددی{\phi} حدیں ہوں گی۔
\end{enumerate}
یوں تکمل درج ذیل ہو گا۔
\begin{align*}
...
\end{figure}
\حل
اس خطے کا حجم \عددی{\iiint\limits_D r^2\sin\theta\dif r\dif \theta \dif \phi} ہو گا۔تکمل کی قیمت معلوم کرنے کے لیے درج ذیل اقدام کرنے ہوں گے۔
\begin{enumerate}[1.]
\item
\ترچھا{خاکہ:}\quad
...
\item
\ترچھا{تکمل کی \عددی{r} حدیں:}\quad
ہم مثبت محور \عددی{z} کے ساتھ \عددی{\theta} زاویہ پر  مبدا سے شعاع \عددی{M}  کھینچتے ہیں اور ساتھ ہی  مستوی \عددی{xy} میں اس کی تظلیل \عددی{L}  کھینچتے ہیں جو مثبت محور \عددی{x} کے ساتھ زاویہ \عددی{\phi} بناتا ہے۔ شعاع \عددی{M} خطہ \عددی{D}میں \عددی{r=0)} سے داخل اور \عددی{r=1} سے خارج ہو گا۔
\item
\ترچھا{تکمل کی \عددی{\theta} حدیں:}\quad
 مخروط \عددی{\theta=\pi/3}  مثبت محور \عددی{z} کے ساتھ  زاویہ \عددی{\pi/3} بناتا ہے۔ یوں شعاع \عددی{M}  زاویہ \عددی{\theta=0} سے \عددی{\theta=\pi/3} تک چل سکتی ہے۔
\item
\ترچھا{تکمل کی \عددی{\phi} حدیں:}\quad
...
\end{align*}
ہو گا۔کروی محدد میں \عددی{x^2+y^2=(r\sin\theta\cos\phi)^2+(r\sin\theta\sin\phi)^2=r^2\sin^2\theta} کی وجہ سے  جمودی معیار اثر
\begin{align*}
I_z&=\iiint (r^2\sin^2\theta)r^2\sin\theta\dif r\dif \theta\dif\phi=\iiint r^4\sin^3\theta\dif r\dif\theta\dif\phi
\end{align*}
ہو گا جس کی قیمت  مثال \حوالہ{مثال_بالکثرت_کروی_کرہ_مخروط} کے خطہ کے لیے درج ذیل ہو گی۔
\begin{align*}
...
نیچے سے مستوی \عددی{z=0}، اوپر سے کرہ \عددی{x^2+y^2+z^2=4}، اور اطراف سے بیلن \عددی{x^2+y^2=1} میں  خطہ \عددی{D}  ملفوف ہے۔نلکی محدد میں  خطہ \عددی{D} کا حجم معلوم کرنے کے لیے تہرا تکمل درج ذیل  تکمل کی ترتیب کے لیے لکھیں۔
\begin{multicols}{3}
\begin{enumerate}[a.]
\item
...
%======================
\ابتدا{سوال}
نیچے سے مخروط  \عددی{z=\sqrt{x^2+y^2}}، اوپر سے قطع مکافی  \عددی{z=2-x^2-y^2}  میں  خطہ \عددی{D}  ملفوف ہے۔نلکی محدد میں  خطہ \عددی{D} کا حجم معلوم کرنے کے لیے تہرا تکمل درج ذیل  تکمل کی ترتیب کے لیے لکھیں۔
\begin{multicols}{3}
\begin{enumerate}[a.]
\item
$\dif z\dif \rho\dif\phi$
...
%====================
\ابتدا{سوال}
نیچے سے مستوی \عددی{z=0}،  اطراف سے بیلن \عددی{\rho=\cos\phi}، اور اوپر سے قطع مکافی سطح \عددی{z=3\rho^2} میں ملفوف خطہ \عددی{D} کے لیے درج ذیل تکمل کی قیمت تلاش کرنے کے لیے  کے تکمل کی  حدیں معلوم کریں۔
\begin{align*}
...
سوال \حوالہ{سوال_کثیر_دائرہ_کار_پر_تہرا_تکمل_لکھنا_الف} تا سوال \حوالہ{سوال_کثیر_دائرہ_کار_پر_تہرا_تکمل_لکھنا_ث}  میں دیے گئے خطہ \عددی{D} پر تکمل \عددی{\iiint_D F(\rho,\phi,z)\dif z\,\rho\dif \rho\dif\phi} کی قیمت حاصل کرنے کے لیے تہرا تکمل لکھیں۔
...
%======================
\ابتدا{سوال}
نیچے سے مخروط   \عددی{z=\sqrt{x^2+y^2}} اور اوپر سے مستوی \عددی{z=1}  کے بیچ خطہ \عددی{D}  کے حجم کا تکمل کروی محدد میں(ا) \عددی{\dif r\dif\theta\dif\phi} ، (ب) \عددی{\dif\theta\dif r\dif \phi} ترتیب    کے لیے  لکھیں۔
\انتہا{سوال}
%=================
سوال \حوالہ{سوال_بالکثرت_کروی_حجم_تلاش_الف} تا سوال \حوالہ{سوال_بالکثرت_کروی_حجم_تلاش_ث} میں  دیے گئے ٹھوس جس کے حجم کے کروی تکمل (ا) کی حدیں تلاش کریں۔ (ب) کروی تکمل حل کرتے  ہوئے جسم کا حجم معلوم کریں۔
...
فرض کریں مستوی \عددی{uv} کے خطہ \عددی{G} کا تبادلہ یک بہ یک مطابقت کی مساوات
...
اس کا جواب: اگر \عددی{g}، \عددی{h} اور \عددی{f} کے جزوی تفارق استمراری ہوں اور \عددی{J(u,v)} (جس پر جلد تبصرہ کیا جائے گا) صرف  تنہا نقطوں پر صفر ہو (اگر صفر ہو بھی)  تب درج ذیل ہو گا۔ 
...
\end{align*}
سے بھی ظاہر کیا جاتا ہے جو ہمیں یاد دلاتا ہے کہ \عددی{x} اور \عددی{y} کی جزوی تفارق سے یعقوبی (مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_تعریف})  حاصل ہوتا ہے۔مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_بدل} کی استخراج آپ کو اعلیٰ احصاء کے نصاب میں ملے گی جس کو یہاں پیش نہیں کیا جائے گا۔
...
\label{شکل_کثیر_چوکور_سے_دائری_خطہ}
\end{figure}

شکل \حوالہ{شکل_کثیر_چوکور_سے_دائری_خطہ} میں دکھایا گیا ہے کہ کس طرح  مستطیل \عددی{G: 0\le r\le 1,\,0\le \theta\le \pi/2} کا تبادلہ مساوات \عددی{x=r\cos\theta} اور \عددی{y=r\sin\theta}  ایک چوتھائی دائرہ \عددی{R}، جس کی سرحد  ربع اول میں مستوی \عددی{xy} پر \عددی{x^2+y^2=1} ہے، میں کرتی ہیں۔
...
مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_بدل} استعمال کرنے کی خاطر ہمیں مستوی \عددی{uv} میں مطابقتی خطہ \عددی{G} اور تبادل کا یعقوبی  معلوم کرنے ہوں  گے۔ انہیں دریافت کرنے کے لیے  ہم مساوات \حوالہ{مساوات_بالکثرت_بدل_مثال} کو \عددی{x} اور \عددی{y} کے لیے \عددی{u}  اور \عددی{v} کی صورت میں حل    کرتے ہیں۔یوں درج ذیل حاصل ہو گا۔
...
\حل
ہم  مستوی{xy} میں تکمل کے خطہ \عددی{R} کا خاکہ بنا  کر اس کی سرحدوں کی نشاندہی کرتے ہیں (شکل \حوالہ{شکل_کثیر_مثلث_سے_مثلث})۔متکمل کو دیکھ کر ہمیں خیال آتا ہے کہ تبادل \عددی{u=x+y} اور \عددی{v=y-2x}استعمال کیا جائے جنہیں \عددی{u} اور \عددی{v} کی صورت میں \عددی{x} اور \عددی{y} کے لیے حل کرتے ہوئے درج ذیل حاصل ہو گا۔
...
نلکی اور کروی بدل ان بدل کی خصوصی صورتیں ہیں جو تہرا تکملات کے متغیرات کی تبدیلی کو تین بُعدی فضا کا تبادل تصور کرتے ہیں۔ یہ ترکیب دوہرا  تکملات کی ترکیب کی طرح ہے، بس اب ہم دو کے بجائے تین بعد میں کام  کرتے ہیں۔ 
\begin{figure}
\centering
\begin{subfigure}{0.45\textwidth}
\centering
...
فرض کریں \عددی{uvw} فضا میں خطہ \عددی{G} کا تبادلہ  یک بہ یک مطابقت کے  ساتھ \عددی{xyz} فضا کے خطہ \عددی{D} میں درج ذیل  روپ کی مساواتوں  سے کیا جاتا ہے (شکل \حوالہ{شکل_کثیر_مکعب_سے_منشور})۔
\begin{align*}
...
\end{align*}
تصور کیا جا سکتا ہے۔اگر \عددی{g}، \عددی{h}  اور \عددی{k} کے اول جزوی تفارق استمراری ہوں تب \عددی{D} پر \عددی{F} کے تکمل کا \عددی{G} پر \عددی{H(u,v,w)} کے تکمل کے ساتھ تعلق درج ذیل مساوات دیگی۔
...
\end{align} 
متغیرات کی تبدیلی کا    کلیہ، جس کو مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_تہرا_تکمل} میں پیش کیا گیا ہے، پیچیدہ ہے اور دو بُعدی صورت کی طرح ، اس کی اشتقاق کو یہاں پیش نہیں کیا جائے گا۔
\begin{figure}
\centering
\begin{subfigure}{0.45\textwidth}
\centering
...
\end{align}
 کروی محدد میں \عددی{0\le \theta\le \pi} کی وجہ سے   \عددی{\sin\theta} کبھی بھی منفی نہیں ہو  سکتا   ہے  لہٰذا مطلق کی علامت لکھنے کی ضرورت نہیں ہے۔ 
...
مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_تہرا_تکمل}  استعمال کرنے کے لیے ہمیں  \عددی{uvw}  فضا میں  مطابقتی خطہ \عددی{G} اور تبادل کا یعقوبی جاننا ہو گا۔ ہم مساوات \حوالہ{مساوات_بالکثرت_کروی_بدل} کو \عددی{x}، \عددی{y} اور \عددی{z} کے لیے \عددی{u}، \عددی{v} اور \عددی{w} کی صورت میں حل کر کے 
...
\end{vmatrix}=6
\end{align*}
ہم  مساوات \حوالہ{مساوات_بالکثرت_یعقوبی_تہرا_تکمل} استعمال کرنے کے لیے درکار تمام معلوم جان چکے ہیں۔ یوں درج ذیل ہو گا۔
\begin{align*}
\int_0^3\int_0^4\int_{x=y/2}^{x=y/2+1}&\big(\frac{2x-y}{2}+\frac{z}{3}\big)\dif x\dif y\dif z\\
...
\begin{enumerate}[a.]
\item
درج ذیل نظام کو \عددی{x} اور \عددی{y} کے لیے \عددی{u} اور \عددی{v} کی صورت میں حل کریں۔ اس کے بعد یعقوبی \عددی{\tfrac{\partial(x,y)}{\partial(u,v)}} کی قیمت تلاش کریں۔
\[u=x-y,\quad v=2x+y\]
\item
...
%===============================
\ابتدا{سوال}
مستوی \عددی{xy} میں   \عددی{\tfrac{x^2}{a^2}+\tfrac{y^2}{b^2}=1}  پر تفاعل \عددی{f(x,y)=1} کا تکمل لے کر ترخیم کا رقبہ \عددی{\pi ab} حاصل کیا جا سکتا ہے۔ اس تکمل کو سیدھا حل کرنے کے لیے اس میں تکونیاتی تفاعل  پر کرنا ہو گا۔ اس سے آسان طریقہ تبادل \عددی{x=au}، \عددی{y=bv} استعمال کرتے ہوئے تبدیل شدہ تکمل کی    قیمت کا مستوی \عددی{uv}     میں  قرص \عددی{G:u^2+v^2\le 1} پر حصول ہے۔
\انتہا{سوال}
%=========================
\ابتدا{سوال}
